...
struct RelationInfo {
    ...
    /// `--native-merge` NATIVE CUSTOM `:merge`: the retained [`MergeFn`] tree for a
    /// custom `:merge` whose body either BUILDS TERMS (a top-level
    /// `IfEq{canon(old), canon(new), …, Seq[<Construct/TableInsert>…, <value>]}`
    /// from `translate_term_build_merge_to_seq`) or is a pure VALUE-FOLD (a fold of
    /// primitives over `old`/`new`, e.g. `(from-string (to-string (* new old)))`).
    /// `add_table` retains it here — instead of collapsing to a 4-variant
    /// [`MergeMode`] — so `resolve_merge` can EXECUTE it host-side
    /// (`resolve_term_build_merge` / `eval_term_build_node`): the value fold is a
    /// side-effect-free pairwise fold, the term build mints e-nodes via
    /// `lookup_or_create`, writes the `@<C>View` rows, and returns the merged
    /// eclass. `None` for every other merge (the [`merge`](Self::merge)
    /// `MergeMode` drives those).
    merge_tree: Option<MergeFn>,
}
...
impl EGraph {
    ...
    fn resolve_merge(&mut self, f: FunctionId, keys: &HashSet<Vec<u32>>) -> bool {
        ...
        // `--native-merge` NATIVE CUSTOM `:merge`: a custom `:merge` whose body
        // builds terms (or is a pure value fold) runs its retained `MergeFn` tree
        // host-side, minting e-nodes and writing `@<C>View` rows. Dispatch to the
        // dedicated interpreter, which folds the conflicting candidate rows pairwise
        // through the tree. Checked FIRST so the retained tree drives the merge even
        // though the function still carries a (sentinel `New`) `MergeMode`.
    ...
    /// `ResolvedMergeFn::run` for the term-build subset. Columns are read via the
    /// uniform `Row` accessor (`row[..]`), so the packed-vs-unpacked detail does not
    /// leak here — the rows are already `u32` reps (see `compile::Row`).
        ...
        match node {
            ...
            MergeFn::Construct(table, key_args, value_args) => {
                ...
                let key: Vec<Value> = key_args
                    ...
                    .collect();
                // A non-empty `value_args` would be a multi-value constructor, which
                // Feldera does not yet support (the single-output `lookup_or_create`
                // only mints the output column).
                assert!(
                    value_args.is_empty(),
                    "Feldera term-build merge: multi-value Construct (value_args) not supported"
                );
            ...
            MergeFn::Columns(cols) => match cols.as_slice() {
                ...
                _ => panic!(
                    "Feldera term-build merge: multi-value Columns ({} cols) not supported",
                    cols.len()
                ),
            },
            MergeFn::AssertEq
...
/// Map a *scalar* (single value column) [`MergeFn`] to the Feldera [`MergeMode`] that resolves one
/// output column's FD conflict. Mirrors the DuckDB/FlowLog recognition:
...
///     and user `:merge (min old new)`).
///   - `OldCol(_)`                => keep the old value
///   - `NewCol(_)`                => keep the new value
///   - `Function`/`Const`         => fall back to `Old`
fn merge_mode_for_scalar(merge: &egglog_backend_trait::MergeFn) -> MergeMode {
...
/// merge and collapses to its one inner column's mode. Feldera's relation model carries exactly
...
/// later step; the term encoder does not emit them on Feldera today.)
fn merge_mode_for_table(merge: &egglog_backend_trait::MergeFn, name: &str) -> MergeMode {
    use egglog_backend_trait::MergeFn;
    match merge {
        MergeFn::Columns(cols) => match cols.as_slice() {
            [single] => merge_mode_for_scalar(single),
            _ => panic!(
                "Feldera backend does not support tuple-output (multi-value) merges yet \
...
fn native_merge_target_uf(merge: &egglog_backend_trait::MergeFn) -> Option<FunctionId> {
    use egglog_backend_trait::MergeFn;
    match merge {
...
/// tree the Feldera host interpreter runs natively (`resolve_term_build_merge`).
///
...
/// defensively. These are the only multi-action shapes Feldera executes; the other
...
/// `:merge` tree the Feldera host interpreter runs natively as a pairwise fold
...
/// `Primitive`/`Const`/`Old`/`New`/`OldCol`/`NewCol`; a single-output function may
/// wrap it in a `Columns([single])`. A value-fold body contains NO `Function` call
/// (a `ResolvedCall::Func` disqualifies it on the frontend — that is a term build),
...
/// `MergeMode` already expresses (e.g. a bare `old`/`new`), so they are NOT retained
/// as a tree — only a fold containing at least one `Primitive` is (everything the
/// plain `MergeMode` cannot express).
fn is_value_fold_tree(merge: &MergeFn) -> bool {
    ...
    match merge {
        ...
        // which Feldera does not support — left to `merge_mode_for_table`).
        MergeFn::Columns(cols) => match cols.as_slice() {
...
impl Backend for EGraph {
    ...
    fn add_table(&mut self, config: FunctionConfig) -> FunctionId {
        ...
        // primitives over `old`/`new`, e.g. `(from-string (to-string (* new old)))`)
        // cannot collapse to a 4-variant `MergeMode` — the term build mints e-nodes
        // + writes view rows, and the value fold computes a value no
        ...
        // (`resolve_merge` dispatches on `merge_tree.is_some()` first); the sentinel
        // `New` keeps a key with >1 candidate eligible (and `merge_mode_for_table`
        // would otherwise panic on a term-build `Seq`/`IfEq`).
        let retain_tree =
            has_output && (is_term_build_tree(&config.merge) || is_value_fold_tree(&config.merge));
        // Recognize the merge mode (FD-conflict resolution); see `merge_mode_for_table`.
    ...
    /// `--native-merge` VALUE-FOLD: Feldera runs a pure value-fold custom `:merge`
    ...
    /// gates the bridge's value-fold-via-callback path that Feldera does not use);
    /// this split capability enables the native value-fold path. Mirrors FlowLog.
    fn supports_value_fold_merge(&self) -> bool {
        true
    }

    /// `--native-merge` TERM-BUILD: Feldera runs a term-building custom `:merge`
    ...
    /// stays `false` (it gates the value-fold-via-callback path that Feldera does
    /// not use); this split capability enables the native term-build path. Mirrors
    /// FlowLog.
