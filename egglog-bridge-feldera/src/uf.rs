//! Native union-find used by the `--native-uf --feldera` path.
//!
//! Copied verbatim from `egglog-bridge-flowlog/src/uf.rs` (itself a verbatim
//! copy of `egglog-bridge-duckdb/src/uf.rs` — the data structure is
//! backend-agnostic). The Feldera host is single-threaded, so a plain `UfTable`
//! suffices — no `Arc<Mutex>` wrapping like the DuckDB UDF read path needs.
//!
...
// `std::collections::HashMap` (rather than the workspace-preferred hashbrown)
// is intentional: this file is a verbatim copy of the DuckDB backend's UF, whose
// internal node maps use std. Allow the disallowed type so the copy stays exact.
...
// Copied verbatim from the DuckDB backend; that backend exercises every method
// via the find UDF + saturate stop-condition, but the single-threaded Feldera
// host only needs a subset (`new`/`find_ro`/`enqueue_union`/`drain_pending`/
// `displaced_len`/`drain_displaced`). Keep the full surface (and silence
// dead-code on the rest) so the file stays a verbatim copy.
#[allow(dead_code)]
impl UfTable {
    pub fn new() -> Self {
        Self::default()
