...
- **`Core::base<T>(&id)` / `Core::id_of<T>(x, sort)`** helpers on the state-wrapper trait. `base` extracts a Rust base value from an `Id`; `id_of` constructs an `Id` from a Rust base value and an egglog sort name. Convenience for primitive-author bodies that want to work in `Id`s without the verbosity of `Id::new(state.base_to_value::<T>(x), "i64")`. The lower-level `value_to_base` / `base_to_value` remain available.
- Primitive trait `apply` signatures (`PurePrim` / `WritePrim` / `ReadPrim` / `FullPrim`) keep `args: &[Value] -> Option<Value>` — primitive authors still work at the `Value` level. Fully hiding `Value` from primitive signatures would require per-signature wrapper plumbing across all dispatch sites; deferred as a follow-up. The typed user API (`Id`-returning) is now the documented headline path; raw `Value` is documented as the primitive-author / advanced surface.
