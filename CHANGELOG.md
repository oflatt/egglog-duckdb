...
- **`EGraph::base<T>(&id) -> T`** — unchecked counterpart to `extract` on the `EGraph` itself. Useful when reading from `table_rows::<Vec<Id>>` rows whose `Id`s carry empty sort tags. The previously public `EGraph::value_to_base` / `EGraph::base_to_value` were demoted to `pub(crate)` — external callers should reach for `intern` / `extract` (sort-checked) or `base` (unchecked) instead.
