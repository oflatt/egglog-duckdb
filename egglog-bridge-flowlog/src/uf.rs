//! Native union-find used by the `--native-uf --flowlog` path.
//!
//! Copied verbatim from `egglog-bridge-duckdb/src/uf.rs` (the data structure is
//! backend-agnostic). The FlowLog host is single-threaded, so a plain `UfTable`
...
// via the find UDF + saturate stop-condition, but the single-threaded FlowLog
