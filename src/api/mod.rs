...
//!
//! For rows whose columns are not base values (eclass ids, container
//! values), use [`Value`] directly or [`RawValues`] as an escape hatch.
//! The Rust API does not currently track egglog sort identity on
//! [`Value`]s — a `Value` returned from a `Math` constructor and a
//! `Value` returned from a `List` constructor are indistinguishable at
//! the Rust type level; it is the caller's responsibility to keep
